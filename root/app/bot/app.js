...
client.on('message', msg => {
  ...
  if (msg.content.startsWith('!faucet')) {
    ...
    userdb.get(userid, function (err, destination) {
      if (err){
        msg.reply(userid + ' has not registered an address please go to ' + walleturl + ' and click Generate Wallet then use !register-address with your address');
      }
      else{
            ...
            if (hasfunds == true){
              if (readytosend == true){
                ...
                if (NanoCurrency.checkHash(sendres.hash)){
                  msg.reply(faucetamount + ' coins have been sent to ' + destination + ' ' + walleturl + 'block/' + sendres.hash);
